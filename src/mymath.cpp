...
mathMethods::mathMethods(void) : r(-4256){
}
...
int mathMethods::pnpoly(int nvert, double *vertx, double *verty, double testx,
		double testy) {
	int i, j, c = 0;
	for (i = 0, j = nvert - 1; i < nvert; j = i++) {
		if (((verty[i] > testy) != (verty[j] > testy)) && (testx < (vertx[j]
				- vertx[i]) * (testy - verty[i]) / (verty[j] - verty[i])
				+ vertx[i]))
			c = !c;
	}
	return c;
...
void mathMethods::K_S_Test(double * data1, unsigned long n1, double * data2,
...
void mathMethods::randomMisorientationAxisConsidered(double * qref,
		double * qr, double maxDev) {
	...
	double refNorm = sqrt(
			SQR(qref[0]) + SQR(qref[1]) + SQR(qref[2]) + SQR(qref[3]));
	...
	while (theta < dev) {
		...
		theta = q[3] * qref[0] + q[2] * qref[1] + q[1] * qref[2] + q[0]
				* qref[3];
	}
	qr[0] = q[3];
	qr[1] = q[2];
...
double mathMethods::misorientationCubicOrigInv(double pa1, double Pa,
		double pa2, double pb1, double Pb, double pb2) {
	int i;
	...
	r0[2][2] = (r[1] + r[2]) * fac;
	r0[2][3] = (r[1] - r[2]) * fac;
	r0[3][0] = (r[0] + r[1] + r[2] + r[3]) * 0.5;
	r0[3][1] = (r[0] + r[1] - r[2] - r[3]) * 0.5;
	r0[3][2] = (r[0] - r[1] + r[2] - r[3]) * 0.5;
	r0[3][3] = (r[0] - r[1] - r[2] + r[3]) * 0.5;
	r0[4][0] = (r[0] + r[1] + r[2] - r[3]) * 0.5;
	r0[4][1] = (r[0] + r[1] - r[2] + r[3]) * 0.5;
	r0[4][2] = (r[0] - r[1] + r[2] + r[3]) * 0.5;
	r0[4][3] = (r[0] - r[1] - r[2] - r[3]) * 0.5;
	r0[5][0] = r[0];
	r0[5][1] = r[1];
	r0[5][2] = r[2];
	r0[5][3] = r[3];
...
double mathMethods::misorientationCubic(double pa1, double Pa, double pa2,
		double pb1, double Pb, double pb2) {
	Real oria[3] = { pa1, Pa, pa2 };
	Real orib[3] = { pb1, Pb, pb2 };
...
double mathMethods::misorientationCubicCOReV2(double pa1, double Pa,
		double pa2, double pb1, double Pb, double pb2) {
	int i;
...
double mathMethods::misorientationCubicCorrectCOReV2InvertAndMult(double pa1,
		double Pa, double pa2, double pb1, double Pb, double pb2) {
	int i;
...
void mathMethods::newOrientationFromReferenceFixedAngularCone(double * oriOri,
		double maxDev, double angle, double u, double v, double w,
		double * newOri) {
	Real qr[4];
	Real ori[4], qideal[4];
	...
	Real qref[4] = { cos(0.5 * angle), u * _norm * sin(0.5 * angle), v * _norm
			* sin(0.5 * angle), w * _norm * sin(0.5 * angle) };
...
void mathMethods::rotateOrientation(double *oriOri, double angle, double u,
		double v, double w, double *newOri) //Angle in radians
{
	Real ori[4], qideal[4];
	Real _norm = 1.0 / sqrt(SQR(u) + SQR(v) + SQR(w));
...
void mathMethods::euler2quaternion(double * euler, double * q) {
	...
	Real p[4] = { co1 * cos((p1 + p2) / 2), s1 * cos((p1 - p2) / 2), s1 * sin(
			(p1 - p2) / 2), co1 * sin((p1 + p2) / 2) }; //applying sin, cos addition theorems
...
void mathMethods::newOrientationFromReference(double *bunge, double deviation,
		double *newOri) {
	Real qrndmisori[4];
...
void mathMethods::devtorefEuler2RGB(double *bunge, double *ideal,
		double maxDev, unsigned char *rgb) {
...
void mathMethods::devtoaxisEuler2RGB(double *bunge, double *uvw, double maxDev,
		unsigned char *rgb) {
