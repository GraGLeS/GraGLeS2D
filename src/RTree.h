...
#define MEMORY_POOL_SIZE	524288
#define MAXIMUM_NUMBER_OF_POOLS 16
/// \class RTree
...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], vector<DATATYPE>& output) const;
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, vector<DATATYPE>& output) const;
  ...
  vector<vector<Node> >	m_nodeMemoryPool;		   ///< Memory pool
  int m_currentNodeCount;						   ///< Node count
};

RTREE_TEMPLATE
RTREE_QUAL::RTree() : m_currentNodeCount(0)
{
...
RTREE_TEMPLATE
void RTREE_QUAL::initMemoryPools()
{
	m_nodeMemoryPool.reserve(MAXIMUM_NUMBER_OF_POOLS);
	m_nodeMemoryPool.push_back(vector<Node>());
	m_nodeMemoryPool[0].reserve(MEMORY_POOL_SIZE);
}
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], vector<DATATYPE>& output) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, output);
...
void RTREE_QUAL::Reset()
{
  ...
  for(unsigned int i=0; i<m_nodeMemoryPool.size(); i++)
  {
	  m_nodeMemoryPool[i].clear();
  }
...
typename RTREE_QUAL::Node* RTREE_QUAL::AllocNode()
{
  ...
  newNode = new Node;
#else // RTREE_DONT_USE_MEMPOOLS
  unsigned int poolID = m_currentNodeCount / MEMORY_POOL_SIZE;
  if(poolID >= m_nodeMemoryPool.size())
  {
	  m_nodeMemoryPool.push_back(vector<Node> ());
	  m_nodeMemoryPool[m_nodeMemoryPool.size()-1].reserve(MEMORY_POOL_SIZE);
  }
  m_nodeMemoryPool[poolID].push_back(Node());
  newNode = &m_nodeMemoryPool[poolID][m_nodeMemoryPool[poolID].size()-1];
  m_currentNodeCount++;
#endif // RTREE_DONT_USE_MEMPOOLS
  InitNode(newNode);
  return newNode;
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, vector<DATATYPE>& output) const
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, output))
        {
