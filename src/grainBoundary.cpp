...
bool ExplicitGrainBoundary::extractBoundaryAndJunctions(DimensionalBufferReal& distanceBuffer, DimensionalBufferIDLocal& idLocal,
														int timestep, bool verbose)
{
	...
	MarchingSquaresAlgorithm marchingSquares(distanceBuffer, idLocal, m_owningGrain);
	bool has_contour = marchingSquares.generateContour(m_grainBoundary, m_grainJunctions);
	...
	if(verbose)
	{
		...
		filename << "ContourLine_"<<m_owningGrain->getID()<<"_Timestep_"<<timestep<<".gnu";
		ofstream file;
		...
		filename.str("");
		filename << "Junctions_"<<m_owningGrain->getID()<<"_Timestep_"<<timestep<<".gnu";
		file.open(filename.str());
		for(unsigned int i = 0; i < m_grainJunctions.size(); i++)
			file << m_grainJunctions[i].coordinates.x << " " << m_grainJunctions[i].coordinates.y << endl;
		file.close();
	}
	return true;
}

void ExplicitGrainBoundary::buildDirectNeighbours(DimensionalBufferReal& distanceBuffer, DimensionalBufferIDLocal& idLocal,
												int timestep, bool verbose)
{
...
catch(const std::out_of_range& oor)
{
		...
		if (Settings::IsIsotropicNetwork) {
			...
			if (Settings::MicrostructureGenMode == E_GENERATE_TESTCASE) {
				m_grainBoundary[i].energy = m_owningGrain->getWeigthFromHandler(m_owningGrain->getID(), candidate->getID());
			} 
			
			else if(Settings::ResearchProject != 0) m_grainBoundary[i].energy=1.0;			
			else {
			...
			if(m_grainBoundary[i].energy != m_grainBoundary[i].energy ) {
				cout << m_owningGrain->getID() << "nan in Energy computation " << endl;
				m_grainBoundary[i].energy = 0.005;
			}
		}
		...
		if (it == m_directNeighbourhood.end()) {
			if (Settings::UseMobilityFactor && (!Settings::IsIsotropicNetwork))
				mu = m_owningGrain->GBmobilityModel(thetaMis);
			else
				mu = 1;
	...
	m_grainBoundary[m_grainBoundary.size() - 1].energy = m_grainBoundary[0].energy;
}
...
double ExplicitGrainBoundary::computeEnergy(int timestep, bool verbose)
{
	...
	for(unsigned int i=0; i<m_grainBoundary.size() - 1; i++)
	{
		total_energy += (m_grainBoundary[i] - m_grainBoundary[i+1]).len() * m_grainBoundary[i].energy * m_owningGrain->get_h();
	}
	return total_energy;
}
...
double ExplicitGrainBoundary::computePerimeter(int timestep, bool verbose)
{
	...
	for(unsigned int i=0; i<m_grainBoundary.size() - 1; i++)
	{
		perimeter += (m_grainBoundary[i] - m_grainBoundary[i+1]).len() * m_owningGrain->get_h();
	}
	return perimeter;
}

void ExplicitGrainBoundary::buildBoundarySectors(DimensionalBufferIDLocal& idLocal, int timestep, bool verbose)
{
	...
	for (unsigned int i = 1; i < m_grainJunctions.size(); i++)
	{
		...
		if (m_constantSectors[currentlyBuilt].isSegmentWithinSector(m_grainBoundary, currentSector.getRightContourPoint()))
		{
			m_constantSectors[currentlyBuilt].mergeWith(&m_grainJunctions[i]);
			m_constantSectors[currentlyBuilt].setLeftContourPoint(currentSector.getLeftContourPoint());
			if (i == m_grainJunctions.size() - 1)
			{
				if (m_constantSectors[0].isSegmentWithinSector(m_grainBoundary, m_constantSectors[currentlyBuilt].getLeftContourPoint()))
				{
					if (currentlyBuilt == 0)
					{
					...
					{
						for ( unsigned int j = 0; j < m_constantSectors[currentlyBuilt].getJunctions().size(); j++)
						{
							m_constantSectors[0].mergeWith(m_constantSectors[currentlyBuilt].getJunctions()[j]);
						}
						m_constantSectors[0].setRightContourPoint(m_constantSectors[currentlyBuilt].getRightContourPoint());
					}
				}
			}
		}
		else
		{
			if (i == m_grainJunctions.size() - 1)
			{
				if (m_constantSectors[0].isSegmentWithinSector(m_grainBoundary, currentSector.getLeftContourPoint()))
				{
					m_constantSectors[0].setRightContourPoint(currentSector.getRightContourPoint());
	...
	if (m_constantSectors.size() > 1 || m_constantSectors[0].getLeftContourPoint() != m_constantSectors[0].getRightContourPoint())
	{
		for (unsigned int i = 0; i < m_constantSectors.size() && m_constantSectors.size() > 1; i++)
		{
			ContourSector& secR = m_constantSectors[i];
			ContourSector& secL = m_constantSectors[(i + 1 + sec_len) % sec_len];

			ContourSector new_right = secR.generateLeftInterpolatingContour(m_grainBoundary, m_owningGrain->get_h());
			ContourSector new_left = secL.generateRightInterpolatingContour(m_grainBoundary, m_owningGrain->get_h());
			bool merged = false;

			for (int j = new_right.getRightContourPoint(); j != PERIODIC(new_right.getLeftContourPoint() + 1); j = PERIODIC(j + 1))
			{
				if (j == new_left.getLeftContourPoint() || j == new_left.getRightContourPoint())
				{
					merged = true;
				}
			}
			if (merged)
			{
				m_interpolatingSectors.push_back(secL.generateInterpolatingSector(secR, m_grainBoundary, m_owningGrain->get_h()));
			}
			else
			{
	...
	if(verbose)
	{
		...
		outputFilename<<"Sectors_"<<m_owningGrain->getID()<<"_Timestep_"<<timestep<<".gnu";
		...
		{
			m_interpolatingSectors[i].debugPrintSector(m_grainBoundary, outputFile);
		}
		outputFile.close();
	}
}

double ExplicitGrainBoundary::getWeight(int i, int j, DimensionalBufferIDLocal& idLocal,  int timestep, bool verbose)
{
	double lambda;
	int segment = projectToGrainBondary(SPoint(j, i, 0), lambda);

	for (unsigned int q = 0; q < m_constantSectors.size(); q++)
		if (m_constantSectors[q].isSegmentWithinSector(m_grainBoundary, segment))
		{
			return m_constantSectors[q].getWeight(m_owningGrain, &m_grainBoundary, segment, lambda);
		}

	for (unsigned int q = 0; q < m_interpolatingSectors.size(); q++)
		if (m_interpolatingSectors[q].isSegmentWithinSector(m_grainBoundary, segment))
		{
			return m_interpolatingSectors[q].getWeight(m_owningGrain, &m_grainBoundary, segment, lambda);
		}
	return m_owningGrain->getGBEnergyTimesGBMobility(i, j);
}

int ExplicitGrainBoundary::projectToGrainBondary(SPoint point, double& out_lambda) const
{
...
void ExplicitGrainBoundary::setPointsOnBoundary(ContourSector& sector)
{
	...
	dist += ((m_grainBoundary[PERIODIC(l_segment + 1)]	- m_grainBoundary[l_segment]) * (1	- sector.getJunctions()[0]->lambda)).len() * m_owningGrain->get_h();
	...
	while (dist < Settings::ConstantSectorRadius)
	{
		...
		dist += (m_grainBoundary[PERIODIC(l_segment + 1)] - m_grainBoundary[l_segment]).len() * m_owningGrain->get_h();
		l_segment = PERIODIC(l_segment + 1);
	}
	dist = 0;
	...
	dist += ((m_grainBoundary[PERIODIC(r_segment + 1)]	- m_grainBoundary[r_segment]) * (sector.getJunctions()[0]->lambda)).len() * m_owningGrain->get_h();
	r_segment = PERIODIC(r_segment);
	while (dist < Settings::ConstantSectorRadius)
	{
		//dist += (m_grainBoundary[PERIODIC(r_segment - 1)] - m_grainBoundary[r_segment]).len();
		dist += (m_grainBoundary[PERIODIC(r_segment - 1)] - m_grainBoundary[r_segment]).len() * m_owningGrain->get_h();
...
characteristics& ExplicitGrainBoundary::getDirectNeighbourCaracteristic(LSbox* neighbour)
{
...
map<int, double>& ExplicitGrainBoundary::getDiscreteEnergyDistribution()
{
	calculateDiscreteEnergyDistribution();
	return m_discreteEnergyDistribution;
}

void ExplicitGrainBoundary::calculateDiscreteEnergyDistribution()
{
	m_discreteEnergyDistribution.clear();
	double dh = Settings::HAGB / Settings::DiscreteSamplingRate;
	for(unsigned int i=0; i<m_directNeighbourhood.size(); i++)
	{
		int key = (int) ((m_directNeighbourhood[i].energyDensity) / dh - 0.5);
		double length = m_directNeighbourhood[i].length;
		map<int,double>::iterator it = m_discreteEnergyDistribution.find(key);
		if(it == m_discreteEnergyDistribution.end())
		{
			m_discreteEnergyDistribution.insert(make_pair(key, length));
		}
		else
		{
			(*it).second += length;
...
double ExplicitGrainBoundary::getMeanM() const
{
	...
	for(unsigned int i=0; i < m_directNeighbourhood.size(); i++)
	{
		avg_m += m_directNeighbourhood[i].directNeighbour->getDirectNeighbourCount();
	}
...
double ExplicitGrainBoundary::getMeanA() const
{
	...
	for(unsigned int i=0; i < m_grainJunctions.size(); i++)
	{
		if(i == m_grainJunctions.size() - 1)
		{
			avg_a += (m_grainJunctions[0].coordinates - m_grainJunctions[i].coordinates).len();
		}
		else
		{
			avg_a += (m_grainJunctions[i].coordinates - m_grainJunctions[i+1].coordinates).len();
		}
	}
